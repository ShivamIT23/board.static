...
export default function MobileTool({
    ...
    const canDraw = isTeacher || drawingEnabled || isClassEnded;
    ...
    // If student has no drawing permission, show a simplified pointer dock
    if (!canDraw) {
        return (
            <div className="fixed bottom-2 left-1/2 z-40 bg-sidebar/95 backdrop-blur-xl border border-border/70 rounded-2xl shadow-2xl p-1.5 flex items-center gap-2 animate-dock-up">
                <button
                    type="button"
                    onClick={() => setTool("select")}
                    className={cn(
                        "w-8 h-8 rounded-lg border transition-all flex items-center justify-center cursor-pointer",
    ...
    return (
        <>
            {/* Hidden File Inputs (Teacher Only) */}
            {isTeacher && (
                <>
            ...
            <div className="fixed bottom-2 left-1/2 z-40 max-w-[96vw] bg-sidebar/95 backdrop-blur-xl border border-border/70 rounded-2xl shadow-2xl p-1.5 flex flex-col gap-1 items-center animate-dock-up">
                ...
                <div className="flex items-center gap-1">
                    ...
                    {/* 16. PDF Upload (Teacher Only - Never shown to student) */}
                    {isTeacher && (
                        <button
                            type="button"
                    ...
                    {/* 17. Background Color (Teacher Only - Never shown to student) */}
                    {isTeacher && (
                        <button
                            type="button"
                    ...
                    {isTeacher && <div className="w-px h-4 bg-border/50 shrink-0" />}
                    ...
                    {/* 20. View Lock (Teacher Only - Never shown to student) */}
                    {isTeacher && onToggleViewLocked && (
                        <button
                            type="button"
                    ...
                    {/* 21. Clear Canvas (Teacher Only or post-class - Never shown to normal student) */}
                    {(isTeacher || isClassEnded) && onClearCanvas && (
                        <button
                            type="button"
                            onClick={async () => {
            ...
            {/* 7. Background Color Modal (Teacher Only) */}
            {activeModal === "bg" && isTeacher && (
                <div className="fixed bottom-22 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
