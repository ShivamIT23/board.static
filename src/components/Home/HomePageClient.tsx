...
const WHITEBOARDZONE_MAIN_URL = "https://whiteboardzone.com";
...
const DEFAULT_TESTIMONIALS = [
  {
    ...
    avatar: "DM",
    text: "WhiteBoardZone Board has completely transformed how I teach geometry and calculus. The real-time vector shapes, coordinate graph axes, and math notation make complex concepts effortless to demonstrate.",
    rating: 5,
  },
  {
...
function HomePageContent() {
  ...
  const [testimonialHeader, setTestimonialHeader] = useState({
    title: "Trusted by 500+ Educators & Academies",
    highlight: "500+",
    subtitle: "Read how tutors, schools, and STEM educators use WhiteBoardZone Board for interactive online classes every day.",
    rating: "4.9/5",
    ...
    fetch("/api/testimonials")
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          setTestimonialsList(
            data.map((item: { name: string; role: string; institution?: string; avatar?: string; text: string; rating?: number }) => ({
              name: item.name,
              role: item.institution ? `${item.role} • ${item.institution}` : item.role,
              avatar: item.avatar || (item.name ? item.name.substring(0, 2).toUpperCase() : "TA"),
              text: item.text,
      ...
      .then((settings) => {
        if (settings) {
          setTestimonialHeader({
            title: settings.testimonialTitle ? `${settings.testimonialTitle} & Academies` : "Trusted by 500+ Educators & Academies",
            highlight: settings.testimonialHighlight || "500+",
            subtitle: settings.testimonialSubtitle || "Read how tutors, schools, and STEM educators use WhiteBoardZone Board for interactive online classes every day.",
            rating: settings.testimonialRating || "4.9/5",
            reviewsCount: settings.testimonialReviewsCount || "480+ reviews",
          });
        }
      })
      .catch(() => {});
  ...
  const handleConnectLink = (e: React.FormEvent) => {
    e.preventDefault();
    if (roomInput.trim()) {
      if (roomInput.includes("whiteboardzone.com/")) {
        window.location.href = roomInput.trim();
      } else {
        window.location.href = `${WHITEBOARDZONE_MAIN_URL}/signup`;
      }
    } else {
      window.location.href = `${WHITEBOARDZONE_MAIN_URL}/signup`;
    }
  ...
  const faqs = [
    {
      q: "What is WhiteBoardZone Board?",
      a: "WhiteBoardZone Board is an interactive online teaching board and virtual classroom platform built for educators, tutors, and schools. It combines a real-time collaborative digital whiteboard, high-precision math and graphing tools, live classroom chat, file sharing, and teacher live audio and video streaming in a single browser-based application."
    },
    {
      q: "How does WhiteBoardZone Board support math, science, and STEM teachers?",
      a: "WhiteBoardZone Board includes specialized tools for STEM instruction, such as customizable coordinate graphing planes, mathematical symbols and notation, directional arrows, geometric shapes (rectangles, ellipses, triangles, parallelograms, stars), and a fluid vector drawing pen and highlighter."
    },
    {
      q: "Can teachers control student permissions during class?",
      a: "Yes. WhiteBoardZone Board features robust role-based access control. Teachers can toggle drawing permissions per student or globally, restrict or enable live chat, approve student admission, lock the student canvas view to follow the teacher, and manage audio/video interactions."
    },
    {
      q: "How do audio, video, and screen sharing work?",
      a: "WhiteBoardZone Board features integrated teacher-only live video and audio broadcasting powered by WebRTC (LiveKit). This one-way video model ensures students can see and hear the instructor with ultra-low latency while keeping the whiteboard front and center without bandwidth bottlenecks."
    },
    {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 font-sans selection:bg-blue-500 selection:text-white relative overflow-x-clip transition-colors duration-200">
      ...
      <div className="absolute inset-x-0 top-0 h-150 overflow-hidden pointer-events-none -z-10">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-275 h-137.5 bg-linear-to-b from-blue-500/10 via-indigo-500/5 to-transparent dark:from-blue-600/15 dark:via-indigo-600/10 blur-3xl pointer-events-none" />
      </div>
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/80 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-1">
            <Image src={logo1} alt="WhiteBoardZone Board Logo" height={56} className="h-14 py-2 w-auto object-contain dark:hidden" priority />
            <Image src={logo} alt="WhiteBoardZone Board Logo" height={56} className="h-14 py-2 w-auto object-contain hidden dark:block" priority />
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href={`${WHITEBOARDZONE_MAIN_URL}/signup`}
      ...
      <section className="pt-12 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
        <div className="text-center space-y-6 max-w-3xl mx-auto">
          ...
          <p className="text-base text-slate-600 dark:text-zinc-400 leading-relaxed max-w-2xl mx-auto">
            WhiteBoardZone Board is a real-time collaborative digital teaching board and virtual classroom designed for educators. Deliver engaging online lessons with interactive whiteboards, precision drawing, math notation, graphing tools, live classroom chat, and teacher audio and video streaming.
          </p>
          ...
          <form onSubmit={handleConnectLink} className="space-y-3 max-w-md mx-auto">
            ...
            <p className="text-[11px] text-slate-500 dark:text-zinc-500">
              ...
              <a href={`${WHITEBOARDZONE_MAIN_URL}`} className="text-blue-600 dark:text-blue-400 font-bold underline">
                whiteboardzone.com
              </a>
            </p>
          </form>
        ...
        <div className="relative max-w-5xl mx-auto pt-4">
          <div className="relative z-10 rounded-[5px] overflow-hidden shadow-2xl shadow-blue-900/10 dark:shadow-blue-900/40 border border-slate-200 dark:border-zinc-800">
            <Image
              src="/home1.png"
              alt="WhiteBoardZone Board - Interactive online whiteboard and virtual classroom for teachers"
              width={1200}
              height={800}
      ...
      <section id="features" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-zinc-900 space-y-12 scroll-mt-20">
        ...
        <div className="text-center pt-4">
          <a
            href={`${WHITEBOARDZONE_MAIN_URL}/register`}
          ...
          >
            Explore All Features on WhiteBoardZone →
          </a>
        </div>
      </section>
      ...
      <section id="drawing" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-zinc-900 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center scroll-mt-20">
        <div className="lg:col-span-6 space-y-6 text-left">
          ...
          <p className="text-sm text-slate-600 dark:text-zinc-400 leading-relaxed">
            WhiteBoardZone Board provides built-in mathematical notation and coordinate axes engineered specifically for tutors and STEM educators. Quickly insert standard notation, Greek characters, and coordinate planes to explain formulas and geometry with total clarity.
          </p>
          ...
          <div className="rounded-[5px] border border-slate-200 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-950 p-3 sm:p-4 shadow-2xl space-y-3 relative">
            <div className="h-80 sm:h-96 bg-white dark:bg-zinc-900 rounded-[5px] border border-slate-200 dark:border-zinc-800 p-4 relative flex items-center justify-center overflow-hidden">
              ...
              <div className="w-full h-full relative flex items-center justify-center">
                {/* Axes */}
                <div className="absolute inset-x-8 top-1/2 h-0.5 bg-slate-300 dark:bg-zinc-700" />
                <div className="absolute inset-y-6 left-1/2 w-0.5 bg-slate-300 dark:bg-zinc-700" />
                ...
                {/* Floating Math Symbols Palette Modal (Matches WhiteBoardZone Board interface) */}
      ...
      <section id="testimonials" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-zinc-900 space-y-12 scroll-mt-20">
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white">
            {testimonialHeader.highlight && testimonialHeader.title.includes(testimonialHeader.highlight) ? (
              <>
                {testimonialHeader.title.split(testimonialHeader.highlight)[0]}
                <span className="text-blue-600 dark:text-blue-400">{testimonialHeader.highlight}</span>
                {testimonialHeader.title.split(testimonialHeader.highlight).slice(1).join(testimonialHeader.highlight)}
              </>
            ) : (
          ...
          <div className="inline-flex items-center gap-1 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-bold mt-2">
            ...
            <span>{testimonialHeader.rating} · {testimonialHeader.reviewsCount}</span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonialsList.map((item, idx) => {
            const isImage = item.avatar && (item.avatar.startsWith("http") || item.avatar.startsWith("/"));
            return (
              <div
                key={idx}
                className="p-6 rounded-[5px] bg-white dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800 shadow-lg space-y-4 text-left hover:border-blue-500/40 transition-all flex flex-col justify-between"
              >
        ...
        <div className="rounded-[5px] bg-linear-to-r from-blue-600 via-indigo-600 to-[#6466F1] p-8 sm:p-14 text-white shadow-2xl text-left space-y-6 relative overflow-hidden">
          ...
          <div className="max-w-2xl space-y-4 relative z-10">
            ...
            <p className="text-xs sm:text-sm text-blue-100 leading-relaxed">
              Stay in full control of your digital classroom. WhiteBoardZone Board gives teachers role-based permissions to grant or revoke drawing access, manage live classroom chat, enable file sharing, approve joining students automatically or manually, and lock student viewport for focused lectures.
            </p>
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              <a
                href={`${WHITEBOARDZONE_MAIN_URL}/login`}
      ...
      {/* ── 7. Section: Traditional Platforms vs WhiteBoardZone.com ────────── */}
      <section id="comparison" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-zinc-900 space-y-12 scroll-mt-20">
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-[5px] bg-blue-500/10 border border-blue-500/30 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider">
            Why Choose WhiteBoardZone
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white leading-tight">
            Traditional Platforms vs <span className="bg-linear-to-r from-blue-600 via-indigo-500 to-blue-600 bg-clip-text text-transparent">WhiteBoardZone.com</span>
          </h2>
          <p className="text-xs sm:text-sm text-slate-600 dark:text-zinc-400 leading-relaxed">
            Generic video meeting tools and basic whiteboards were never designed for active teaching. Here is how WhiteBoardZone Board delivers the purpose-built features educators actually need.
          </p>
        </div>
          ...
          <div className="p-6 sm:p-8 rounded-[5px] bg-white dark:bg-zinc-900/90 border-2 border-blue-500 dark:border-blue-500/80 shadow-2xl shadow-blue-500/10 space-y-6 text-left relative">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 pb-4">
              <div>
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">WhiteBoardZone Board</h3>
                <p className="text-xs text-slate-500 dark:text-zinc-400">board.whiteboardzone.com</p>
              </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto text-center space-y-6">
        <h2 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">
          Start Teaching on WhiteBoardZone Board Today
        </h2>
      ...
      <footer className="border-t border-slate-200 dark:border-zinc-900 bg-white dark:bg-zinc-950 text-slate-600 dark:text-zinc-400 py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-200">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 pb-10 border-b border-slate-200 dark:border-zinc-900">
          ...
          <div className="space-y-3 text-left">
            <div className="flex items-center gap-2">
              <Image src={logo1} alt="WhiteBoardZone Board Logo" height={26} className="h-6.5 w-auto object-contain dark:hidden" />
              <Image src={logo} alt="WhiteBoardZone Board Logo" height={26} className="h-6.5 w-auto object-contain hidden dark:block" />
            ...
            <p className="text-[11px] leading-relaxed text-slate-500 dark:text-zinc-500 max-w-sm">
              WhiteBoardZone Board is an interactive online teaching board and virtual classroom platform for tutors, schools, and academies—empowering educators with real-time whiteboarding, math graphing, and live engagement tools.
            </p>
          </div>
          ...
          <div className="space-y-2 text-left">
            ...
            <ul className="space-y-1.5 text-[11px]">
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}`} className="hover:text-blue-600 dark:hover:text-white transition-colors">WhiteBoardZone Home</a></li>
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}/login`} className="hover:text-blue-600 dark:hover:text-white transition-colors">Teacher Sign In</a></li>
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}/signup`} className="hover:text-blue-600 dark:hover:text-white transition-colors">Create Free Account</a></li>
            </ul>
          </div>
          ...
          <div className="space-y-2 text-left">
            ...
            <ul className="space-y-1.5 text-[11px]">
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}/terms`} className="hover:text-blue-600 dark:hover:text-white transition-colors">Terms of Service</a></li>
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}/privacy`} className="hover:text-blue-600 dark:hover:text-white transition-colors">Privacy Policy</a></li>
              <li><a href={`${WHITEBOARDZONE_MAIN_URL}/refund`} className="hover:text-blue-600 dark:hover:text-white transition-colors">Refund Policy</a></li>
            </ul>
          </div>
          ...
          <div className="space-y-2.5 text-left">
            ...
            <ul className="space-y-2.5 text-[11px]">
              ...
              <li className="flex items-center gap-2">
                ...
                <a href="mailto:whiteboardzone26@gmail.com" className="hover:text-blue-600 dark:hover:text-white transition-colors">whiteboardzone26@gmail.com</a>
              </li>
              <li className="flex items-center gap-2">
        ...
        <div className="max-w-7xl mx-auto pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-slate-500 dark:text-zinc-500">
          <p>© {new Date().getFullYear()} WhiteBoardZone. All rights reserved.</p>
  ...
  return (
    <Suspense
      fallback={
        <div className="flex flex-col items-center justify-center h-screen bg-zinc-950 text-white gap-4">
          <Loader2 className="animate-spin text-blue-500" size={40} />
          <p className="text-sm font-bold tracking-widest uppercase opacity-80">Loading WhiteBoardZone Board...</p>
        </div>
      }
    >
      <HomePageContent />
