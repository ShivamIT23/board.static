...
interface QuizModalProps {
    ...
    classId?: number
    userName?: string
    onLaunchQuiz?: (newQuiz: QuizState) => void
    onEndQuiz?: () => void
    onDeleteQuiz?: () => void
...
export const DEFAULT_DEMO_QUESTIONS: FetchedQuizQuestion[] = [
    {
        id: "q-1",
        question: "Sample Question 1: Which feature allows collaborative screen sharing in WhiteBoardZone?",
        options: ["Synced View Lock", "PDF Stamp", "Cloud Screen Recorder", "Emoji Stamps"],
        correctOption: 2,
    },
    {
        id: "q-2",
        question: "Sample Question 2: What is the default background color for the whiteboard?",
        options: ["#FFFFFF (Pure White)", "#18181b (Dark Zinc)", "#000000 (Black)"],
        correctOption: 1,
    },
]

export default function DemoQuizModal({
    isOpen,
    onClose,
    role,
    currentQuiz,
    ...
    const [fetchedQuestions, setFetchedQuestions] = useState<FetchedQuizQuestion[]>(DEFAULT_DEMO_QUESTIONS)
    ...
    useEffect(() => {
        ...
        const fetchQuiz = async () => {
            ...
            try {
                ...
                const data = await res.json()
                setFetchedQuestions(data.questions && data.questions.length > 0 ? data.questions : DEFAULT_DEMO_QUESTIONS)
            } catch (err: unknown) {
                console.error("Fetch quiz error:", err)
                setFetchedQuestions(DEFAULT_DEMO_QUESTIONS)
            } finally {
                setIsLoading(false)
            }
        }
    ...
    const handleLaunchQuiz = () => {
        const questionsToUse = fetchedQuestions.length > 0 ? fetchedQuestions : DEFAULT_DEMO_QUESTIONS
        if (onLaunchQuiz) {
            const now = Date.now()
            const newQuiz: QuizState = {
                id: `demo-quiz-${now}`,
                questions: questionsToUse.map((q, qIdx) => ({
                    id: `q-${qIdx}`,
                    question: q.question,
                    options: q.options.map((opt, oIdx) => ({
                        id: `opt-${qIdx}-${oIdx}`,
                        text: opt,
                        votes: oIdx === q.correctOption ? ["student-1", "student-2"] : ["student-3"],
                    })),
                    correctOption: q.correctOption,
                })),
                isActive: true,
                createdAt: now,
                submittedUsers: ["student-1", "student-2", "student-3"],
            }
    ...
    const handleShareQuizLink = async () => {
        setSharedToken("demo-token-123")
        Swal.fire({
            icon: "info",
            title: "Demo Mode",
            text: "Sharing quiz links in live chat is enabled in full classroom packages.",
    ...
    const handleSubmitQuiz = () => {
        if (!currentQuiz || studentHasSubmitted) return
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs animate-in fade-in duration-200">
            <div className="relative w-135 max-w-[94vw] bg-card border border-border rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] transition-all duration-300">
                ...
                <div className="p-5 overflow-y-auto space-y-6 flex-1">
                    {/* Demo Class Disclaimer Notice */}
                    <div className="p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-bold flex items-center gap-2">
                        <AlertCircle size={16} className="shrink-0 text-amber-500" />
                        <span>Note: Quiz feature is for demonstration only and is not part of this demo class.</span>
                    </div>
