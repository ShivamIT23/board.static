  ...
  useEffect(() => {
    ...
    async function loadQuiz() {
      try {
        ...
        const localData = localStorage.getItem(`whiteboardzone_quiz_result_${token}`)
        if (localData) {
          try {
  ...
  const triggerSubmitQuiz = useCallback(async (forceWithCurrentAnswers = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem(`whiteboardzone_quiz_result_${token}`, JSON.stringify(resultPayload))
